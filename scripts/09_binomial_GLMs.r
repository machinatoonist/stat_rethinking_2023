...
dens(inv_logit(rnorm(1e4, 0, 10)) ,
     lwd = 4 ,
     col = 2 ,
     xlab = "probability of event")
...
for (i in 0:n_points) {
    ...
    the_pf <- function(samp, step) {
        ...
        for (l in c(0.5, 0.8, 0.95))
            the_contour[[concat(l)]] <-
            ellipse(cov(pp), centre = pm, level = l)
        if (length(ellipse_hist) > 0 & samp == 1) {
            mixf <- (NSTEPS - step) / NSTEPS
            for (l in c(0.5, 0.8, 0.95)) {
                #lines(ellipse_hist[[i]],col=col.alpha(1,0.3),lwd=1)
                the_contour[[concat(l)]] <-
                    (1 - mixf) * the_contour[[concat(l)]] + mixf * ellipse_hist[[concat(l)]]
            }
        }
        ...
        thelwd <- 2
        if (samp == 1)
            thelwd <- thelwd + 4 * (NSTEPS - step) / NSTEPS
        for (l in c(0.5, 0.8, 0.95))
    ...
    for (l in c(0.5, 0.8, 0.95))
        ellipse_hist[[concat(l)]] <-
        ellipse(cov(pp), centre = colMeans(pp), level = l)
...
x <-
    as.data.frame(cbind(A = dat_sim$A , G = dat_sim$G , D = dat_sim$D))
head(x, 20)
