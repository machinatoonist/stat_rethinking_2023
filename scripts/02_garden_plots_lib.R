...
wedge <- function(dist,
    ...
    if (!missing(alpha)) {
        pt <- sapply(1:length(pt) , function(i)
            col.alpha(pt[i], alpha[i]))
        border <-
            sapply(1:length(pt) , function(i)
                col.alpha(border[i], alpha[i]))
    }
    for (i in 1:n) {
        points.save[i, ] <-
            point.polar(
...
garden <-
    function(arc ,
        ...
        if (missing(adj.cex))
            adj.cex = rep(1, length(data))
        if (missing(adj.lwd))
            adj.lwd = rep(1, length(data))
        ...
        if (missing(ring_dist)) {
            ...
            if (N > 1)
                for (i in 2:N)
                    ring_dist[i] <- ring_dist[i - 1] * goldrat
            ring_dist <- ring_dist / sum(ring_dist)
            ring_dist <- cumsum(ring_dist)
        }
        
        if (length(alpha.fade) == 1)
            alpha.fade <- rep(alpha.fade, N)
            ...
            function(r,
                ...
                if (N > r) {
                    ...
                    for (j in 1:n_poss) {
                        ...
                        new_arc <- c(arc2[1] + span * (j - 1) , arc2[1] + span *
                                         j)
                        pts2 <-
                            draw_wedge(r + 1, hit[j], new_arc, hedge, lines_to = pts[j, ])
                    }#j
                }# N>r
                ...
                if (!missing(lines_to)) {
                    for (k in 1:n_poss) {
                        ...
                        line.short(
                            c(lines_to[1], pts[k, 1]) ,
                            c(lines_to[2], pts[k, 2]) ,
                            lwd = lwd * adj.lwd[r] ,
                            short = 0.04 ,
                            col = col.alpha("black", alpha_l[k])
                        )
                    }
                } # lines_to
