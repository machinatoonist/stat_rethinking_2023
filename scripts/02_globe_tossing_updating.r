...
if (FALSE) {
    ...
    globe_toss <- function(plot = FALSE) {
        lat <- runif(1, -70, 70)
        lon <- runif(1, -180, 180)
        pts <- st_as_sf(data.frame(lon, lat) ,
                        coords = 1:2,
                        crs = 4326)
        if (plot == TRUE)
            globeearth(eye = list(lat, lon))
        return(c(lat, lon, is.na(as.numeric(
            st_intersects(pts, world)
        ))))
    }
...
myglobeearth <-
    function(gdata, runlen, eye, top, ..., do.plot = TRUE)
    {
        xylim <- 0.8
        if (missing(gdata)) {
...
globe_toss <- function(plot = FALSE) {
    ...
    if (plot == TRUE)
        myglobeearth(eye = list(lon, lat))
    return(c(lon, lat, is.na(as.numeric(
        st_intersects(pts, world)
    ))))
}
...
for (i in 1:n) {
    ...
    route <-
        make_route(lonlat_old[1:2] , lonlat[1:2] , steps = mpts)
    #print(route)
    
    # spin globe
    for (j in 1:nrow(route)) {
        myglobeearth(eye = list(route[j, 1] , route[j, 2]) ,
                     col = gcol ,
                     lwd = glwd)
        
        pf(dmax + 0.1)
        if (n > 1)
            draw_samples(samples_old)
        lines(
            x ,
            f(x, a_old, b_old) ,
            lty = 2 ,
        ...
        if (do_save == TRUE) {
            quartz.save(
                concat("frames/frame_", 1000 + nf, ".png") ,
                type = "png",
                device = dev.cur(),
                dpi = 200
            )
    ...
    for (j in 1:mpts) {
        # plot globe
        myglobeearth(
            eye = list(toss_list[1] , toss_list[2]) ,
            col = gcol ,
            lwd = glwd
        )
        ...
        lines(
            ...
            f(x, aseq[j], bseq[j]) ,
            lwd = 5 ,
            col = ifelse(toss == 1, kcols[2], kcols[1])
        )
...
polygon(c(z$x[z$x >= lo &
                  z$x <= hi], hi, lo), c(z$y[z$x >= lo &
                                                 z$x <= hi], 0, 0), col = 2 , border = 1)
